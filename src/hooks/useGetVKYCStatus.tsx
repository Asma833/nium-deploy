...
export const useGetVKYCStatus = () => {
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data?.success) {
        toast.success('VKYC status retrieved successfully');
      }
    },
    onError: (error: any) => {
      const errorMessage = error?.response?.data?.message || error?.message || 'Failed to retrieve VKYC status';
